...
int n;
int dp[100009][3];
int recurse(int i,int last_len) {
    if(i == n + 1) return 1;
    if(dp[i][last_len] !=-1) return dp[i][last_len];
    int res = 0;
    if(last_len == 1) {
        res = ((2*recurse(i+1,1))%MOD + recurse(i+1,2))%MOD;
    }
    if(last_len == 2) {
        res = (2*recurse(i+1,1))%MOD;
    }
    return dp[i][last_len] = res;
}
void solve() {
    cin >> n;
    memset(dp,-1,sizeof dp);
    cout << (3*recurse(2,1))%MOD <<endl;
